...
class ResNetLinear(nn.Module):
    def __init__(self, block, num_classes=1000):
        ...
        while num_classes % world_size != 0:
            num_classes += 1
        self.fc = ColumnParallelLinear(512 * block.expansion, num_classes, input_is_dp=True)
...
class ResNet_MP(nn.Module):
    def __init__(self, block, layers, **kwargs):
        super().__init__()
        self.features = ResNetFeatures(block, layers, **kwargs)
        self.classifier = ResNetLinear(block)
...
def get_resnet50_mp(cuda=True):
    model = ResNet_MP(Bottleneck, [3, 4, 6, 3])
    model.train()
    if cuda:
        model.cuda()
    return model
